class StaffScheduleApp {
  ...
  updateAvailabilitySummary() {
    ...
    while (idx < dateKeys.length) {
      const currDateStr = dateKeys[idx];
      const curr = shiftMap.get(currDateStr);
      const currDate = new Date(currDateStr);

      if (curr[DAY]) dayCount++;
      if (curr[NIGHT]) nightCount++;
      if (curr[DAY] && isWeekendOrFriNight(currDate, DAY)) weekendCount++;
      if (curr[NIGHT] && isWeekendOrFriNight(currDate, NIGHT)) weekendCount++;

      const nextDateStr = dateKeys[idx+1];
      const nextDayObj = nextDateStr ? new Date(nextDateStr) : null;
      const dateDiff = nextDayObj ? ((nextDayObj - currDate) / (1000*3600*24)) : null;
      const next = nextDateStr ? shiftMap.get(nextDateStr) : null;

      if (curr[DAY] && curr[NIGHT]) {
        // Day+Night same day = 1 shift, but look if next day also has Day
        if (
          next &&
          dateDiff === 1 &&
          next[DAY]
          ...
          adjustedTotal += 1;
          idx++; // Next day will be counted as another, so move ahead one step
        } else {
          adjustedTotal += 1;
        }
      } else if (curr[DAY]) {
        // Day only, previous night not available
        if (!((idx > 0) && shiftMap.get(dateKeys[idx-1])[NIGHT])) {
          adjustedTotal += 1;
        }
      } else if (curr[NIGHT]) {
        // Night only, if next day Day is available or next day Night is available do not increment
        if (
          !(next && dateDiff === 1 && (next[DAY] || next[NIGHT]))
        ) {
          adjustedTotal += 1;
        }
      }
