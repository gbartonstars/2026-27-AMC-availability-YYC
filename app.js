  ...
  createAvailabilityDropdown(dateStr, shiftType) {
    ...
    select.addEventListener('change', () => {
      ...
      if (!this.allAvailability[activeStaff][dateStr]) {
        this.allAvailability[activeStaff][dateStr] = { Day: '', Night: '' };
      }
      ...
      // If selecting Training or Vacation, enforce no adjacent Availability
      if (newVal === 'T' || newVal === 'V') {
        if (shift === 'Day') {
          // Cannot have Availability night before or night of this training/vacation day
          if (prevNight === 'A' || thisNight === 'A') {
            alert('You cannot select Training/Vacation on this day while night availability exists immediately before or after. Please remove that availability first.');
            restoreOldValue();
            return;
          }
        } else if (shift === 'Night') {
          // Training/Vacation night: cannot have Availability Day this day or next day
          if (thisDay === 'A' || nextDay === 'A') {
            alert('You cannot select Training/Vacation on this night while day availability exists immediately before or after. Please remove that availability first.');
            restoreOldValue();
            return;
          }
        }
      ...
      // If selecting Availability, enforce no adjacent T/V
      if (newVal === 'A') {
        if (shift === 'Day') {
          // Cannot have T/V on previous Night or this Night
          if (
            prevNight === 'T' || prevNight === 'V' ||
            thisNight === 'T' || thisNight === 'V'
          ) {
            alert('You cannot make yourself available for this Day because there is Training/Vacation on the immediately-adjacent Night. Adjust that first.');
            restoreOldValue();
            return;
          }
        } else if (shift === 'Night') {
          // Cannot have T/V on this Day or next Day
          if (
            thisDay === 'T' || thisDay === 'V' ||
            nextDay === 'T' || nextDay === 'V'
          ) {
            alert('You cannot make yourself available for this Night because there is Training/Vacation on the immediately-adjacent Day. Adjust that first.');
            restoreOldValue();
            return;
          }
        }
      ...
      // Save the new value
      this.allAvailability[activeStaff][dateStr][shift] = newVal;
