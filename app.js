class StaffScheduleApp {
  ...
  saveAllData() {
    ...
    localStorage.setItem('star1ScheduleData', JSON.stringify(payload));
  }

  loadAllData() {
    const raw = localStorage.getItem('star1ScheduleData');
    if (!raw) return;
    try {
  ...
  getOverviewCountsForDate(dateStr) {
    ...
    Object.keys(this.allAvailability).forEach(staff => {
      ...
      if (entry.Day === 'A' || entry.Day === 'V') {
        if (isRN) rnDay++;
        if (isPara) paraDay++;
      }
      if (entry.Night === 'A' || entry.Night === 'V') {
        if (isRN) rnNight++;
        if (isPara) paraNight++;
      }
    });
  ...
  updateAvailabilitySummary() {
    if (this.isOverviewMode) {
      ...
      summary.textContent =
        'Overview mode: RN and Paramedic availability counts (including vacation) are shown inside each day cell. Days with any zero coverage are highlighted with a red border.';
