class StaffScheduleApp {
  ...
  renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (this.isOverviewMode) {
        ...
        // If any shift is zero, mark the cell red
        if (
          counts.rnDay === 0 ||
          counts.rnNight === 0 ||
          counts.paraDay === 0 ||
  ...
  createAvailabilityDropdown(dateStr, shiftType) {
    ...
    select.addEventListener('change', () => {
      ...
      if (!this.allAvailability[staff]) this.allAvailability[staff] = {};
      if (!this.allAvailability[staff][date]) {
        this.allAvailability[staff][date] = { Day: '', Night: '' };
      }
      this.allAvailability[staff][date][shift] = val;
  ...
  // Count RN / Paramedic availability for overview mode
  ...
  updateAvailabilitySummary() {
    // Overview mode: show generic message and skip per-person minimum logic
    if (this.isOverviewMode) {
      document.getElementById('availabilitySummary').textContent =
      ...
      document.getElementById('availabilitySummary').classList.remove('highlight-red','highlight-green');
      return;
    }
    ...
    // Adjusted availability calculation (your rules)
    let adjustedTotal = 0;
    let i = 0;
