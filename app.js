class StaffScheduleApp {
  constructor() {
    ...
    this.privilegedUsers = new Set(["Greg Barton", "Graham Newton", "Scott McTaggart"]);
    this.privilegedAuthenticated = false;

    this.availabilityViewers = new Set(["Greg Barton", "Scott McTaggart", "Graham Newton", "Dave Allison"]);

    this.loginCodes = {
      "Greg Barton": "B123", "Scott McTaggart": "S456", "Dave Allison": "D789",
      "Mackenzie Wardell": "M321", "Chad Hegge": "C654", "Ken King": "K987",
    ...
    this.paramedics = ["Greg Barton","Scott McTaggart","Dave Allison",
      "Mackenzie Wardell","Chad Hegge","Ken King","John Doyle","Bob Odney"];

    this.nurses = ["Graham Newton","Stuart Grant","Kellie Ann Vogelaar","Michelle Sexsmith",
      "Carolyn Hogan","Kris Austin","Flo Butler","Jodi Scott","Janice Kirkham"];
    ...
    this.octokit = new Octokit({auth: "YOUR_GITHUB_OAUTH_TOKEN"}); // replace with actual token
  ...
  showViewAvailabilityButtonIfAllowed() {
    ...
    const panel = document.getElementById("availabilityOverviewPanel");
  ...
  async showAvailabilityOverview() {
    ...
    let panel = document.getElementById("availabilityOverviewPanel");
    if(!panel){
      panel=document.createElement("div");
      panel.id="availabilityOverviewPanel";
      panel.style.marginTop="20px";
      panel.style.textAlign="center";

      const btn=document.createElement("button");
      btn.textContent="Close Overview";
      ...
      document.getElementById("scheduleSection").appendChild(panel);
    }
    panel.style.display="block";

    if(this.charts.availabilityChart) this.charts.availabilityChart.destroy();
    ...
    this.charts.availabilityChart=new Chart(ctx,{
      ...
      options:{
        responsive:true,
        plugins:{legend:{position:"top"}, title:{display:true, text:"Staff Availability Overview"}},
  ...
  async loadAvailability(staffName){
    ...
    try{
      const response=await this.octokit.repos.getContent({
        owner:this.repoOwner, repo:this.repoName, path
      });
  ...
  updateAvailabilitySummary(){
    ...
    Object.entries(data).forEach(([key,val])=>{
      if(val==="A"){
        ...
        if(date.getDay()===0||date.getDay()===6)totalWeekendShifts++;
      }
    });

    document.getElementById("availabilitySummary").textContent=`Day Shifts Available: ${totalDayShifts} | Night Shifts Available: ${totalNightShifts} | Weekend Shifts Available: ${totalWeekendShifts}`;
  }
}

window.onload=()=>new StaffScheduleApp();
