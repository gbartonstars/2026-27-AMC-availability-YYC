  ...
  updateAvailabilitySummary() {
    ...
    selects.forEach(sel => {
      if (sel.value === 'A') {
        ...
        if (!availabilityMap.has(dateStr)) availabilityMap.set(dateStr, { Day: false, Night: false });
        availabilityMap.get(dateStr)[shift] = true;
      }
    });
    let dayCount = 0;
    let nightCount = 0;
    ...
    let prevDate = null;
    let prevShift = null;
    const isWeekendOrFriNight = (date) => {
    ...
    for (const dateStr of sortedDates) {
      ...
      if (availability.Day) dayCount++;
      if (availability.Night) nightCount++;
      if (isWeekendOrFriNight(dateObj)) {
        if (availability.Day) weekendCount++;
        if (availability.Night) weekendCount++;
      }
      ['Day', 'Night'].forEach(shift => {
        if (!availability[shift]) return;
        if (!prevDate) {
        ...
        const diffDays = (currTime - prevTime) / (1000 * 3600 * 24);
        let increment = true;
        if (diffDays === 0) {
          if ((prevShift === 'Day' && shift === 'Night') || (prevShift === 'Night' && shift === 'Day')) increment = false;
        }
        else if (diffDays === 1) {
          if (prevShift === 'Night' && (shift === 'Day' || shift === 'Night')) increment = false;
        }
        else if (diffDays === -1) {
          if (prevShift === 'Day' && shift === 'Night') increment = false;
        }
        if (increment) adjustedAvailabilityCount++;
        prevDate = dateObj;
        prevShift = shift;
      });
    }
    ...
    summaryDiv.innerHTML = `
      ...
      Weekend shifts available: ${weekendCount} | Adjusted total availability: ${adjustedAvailabilityCount}
    `;
  }
}
