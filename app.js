class StaffScheduleApp {
  constructor() {
    ...
    this.octokit = new Octokit({
      auth: 'YOUR_GITHUB_OAUTH_TOKEN' // Replace with your token
    });
  ...
  showViewAvailabilityButtonIfAllowed() {
    ...
    if (this.availabilityViewers.has(this.currentStaff)) {
      btn.style.display = 'block';
    } else {
    ...
    labels.forEach(staff => {
      ...
      Object.entries(schedule).forEach(([key, val]) => {
        if (val === 'A') {
          ...
          if (dt.getDay() === 0 || dt.getDay() === 6) weekend++;  // Sunday=0; Saturday=6
        }
      });
      dayCounts.push(day);
